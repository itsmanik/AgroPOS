...
const PurchasingTable = ({ classname, purchasingItems, changeUnitPrice, reduceItemFromBill, changeQuantity }) => {
  return (
    <div className={classname}>
      ...
      <div className="rounded-md bg-white overflow-scroll overflow-x-hidden p-3 shadow flex-grow">
        <div className="bg-primary text-white grid grid-cols-[2rem,1fr,5rem,5rem,8rem,5rem,5rem,5rem,2rem] grid-rows-1 rounded-lg px-2 text-sm">
        ...
        <div className="flex flex-col gap-1 mt-2">
          {purchasingItems.map((item, index) => {
            return <PurchasingTableItem reduceItemFromBill={reduceItemFromBill} changeQuantity={changeQuantity} key={index} id={item.id} slNo={index + 1} hsn={item.hsn_code} name={item.name} sp={item.selling_price} mrp={item.mrp} gst={item.gst} qty={item.qty} changeUnitPrice={changeUnitPrice} unit={item.unit} />
          })}
        </div>
      </div>
    </div>

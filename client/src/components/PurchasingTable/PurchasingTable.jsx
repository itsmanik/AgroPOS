...
const PurchasingTable = ({ classname, purchasingItems, changeUnitPrice, reduceItemFromBill }) => {
  return (
      ...
      <div className="rounded-md bg-white overflow-scroll overflow-x-hidden p-3 shadow flex-grow">
        ...
        <div className="flex flex-col gap-1 mt-2">
          {purchasingItems.map((item, index) => {
            return <PurchasingTableItem reduceItemFromBill={reduceItemFromBill} key={index} id={item.id} slNo={index + 1} hsn={item.hsn_code} name={item.name} sp={item.selling_price} mrp={item.mrp} gst={item.gst} qty={item.qty} changeUnitPrice={changeUnitPrice} />
          })}
        </div>
      </div>
    </div>
